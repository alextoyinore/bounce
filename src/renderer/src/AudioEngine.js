...
function createEffectNodes(ctx, type) {
  ...
  if (type === 'reverb') {
    ...
    feedbackNode.gain.value = params.feedback
    allpasses.forEach((ap) => {
      ap.frequency.value = params.baseFreq
    })
    lfo.start()
  ...
  async startRecording(trackId) {
    ...
    try {
      const constraints = {
        video: false,
        audio:
          this.inputDeviceId && this.inputDeviceId !== 'default'
...
function playNoteOffline(
  ...
  if (gen.type === 'sampler') {
    ...
    filter.frequency.setValueAtTime(
      gen.params.cutoff !== undefined ? gen.params.cutoff : 20000,
      now
    )
