  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('mouseover', (e) => {
      let target = e.target.closest(
        '[data-help], [title], button, select, input, .track-header, .mixer-channel, .plugin-knob, .clip, .pr-key'
      )
      if (!target) return
    ...
    const dawHistory = {
      ...
      pushState(actionName) {
        ...
        if (this.undoStack.length > 0 && this.undoStack[this.undoStack.length - 1].state === state)
          return
        ...
        try {
          ...
          if (_activeBrowserTab) {
            document
              ...
              .forEach((t) => t.classList.toggle('active', t.dataset.tab === _activeBrowserTab))
            document
              .querySelectorAll('.browser-tab-content')
              .forEach((c) =>
                c.classList.toggle('active', c.id === `browser-${_activeBrowserTab}-tab`)
              )
          }
          requestAnimationFrame(() => {
            if (_seqContainer) {
              _seqContainer.scrollLeft = _seqSL
              _seqContainer.scrollTop = _seqST
            }
            if (_prViewport) {
              _prViewport.scrollLeft = _prSL
              _prViewport.scrollTop = _prST
            }
          })
        } catch (e) {
          isRestoringHistory = false
    ...
    document.getElementById('menu-install-pack')?.addEventListener('click', async (e) => {
      ...
      try {
        ...
        if (result && result.success) {
          ...
          if (libraryBase) {
            const newPackPath =
              libraryBase.endsWith('/') || libraryBase.endsWith('\\')
    ...
    const playSvg =
      '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 256 256"><path d="M232.4,114.49,88.32,26.35a16,16,0,0,0-24.32,13.65v176a16,16,0,0,0,24.32,13.65l144.08-88.14A16,16,0,0,0,232.4,114.49ZM80,216V40l144,88Z"></path></svg>'
    const pauseSvg =
      '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 256 256"><path d="M216,48V208a16,16,0,0,1-16,16H160a16,16,0,0,1-16-16V48a16,16,0,0,1,16-16h40A16,16,0,0,1,216,48ZM96,32H56A16,16,0,0,0,40,48V208a16,16,0,0,0,16,16H96a16,16,0,0,0,16-16V48A16,16,0,0,0,96,32ZM200,208V48H160V208ZM80,208V48H56V208Z"></path></svg>'
    const fileSvg =
    ...
    async function stopAllRecordings() {
      ...
      for (const trackId of ids) {
        ...
        if (result && result.audioBuffer) {
          await createRecordedClip(
            trackId,
            recordingStartsByTrack[trackId] || 0,
            result.audioBuffer,
            result.blob
          )
        }
      ...
      header.addEventListener('click', async (e) => {
        ...
        if (!loaded) {
          ...
          if (!entries || entries.length === 0) {
            content.innerHTML =
              '<li class="browser-item empty-state" style="justify-content: center; opacity: 0.5;">Empty</li>'
          } else {
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (
        ...
        const snapDuration =
          sequencer.gridSnapInBeats > 0
            ? sequencer.gridSnapInBeats / beatsPerSecond
            : 0.25 / beatsPerSecond
        ...
        if (selectedNotes.size > 0) {
          ...
          selectedNotes.forEach((noteEl) => {
            ...
            if (row) {
              ...
              const stepSeqRow = document.querySelector(
                `.seq-steps[data-instrument="${currentTrackId}"][data-note="${noteName}"]`
              )
              if (stepSeqRow) {
            ...
            if (row) {
              ...
              if (newIndex >= 0 && newIndex < ALL_NOTES_DESC.length) {
                ...
                if (newRow) {
                  ...
                  const stepSeqRow = document.querySelector(
                    `.seq-steps[data-instrument="${currentTrackId}"][data-note="${newNoteName}"]`
                  )
                  if (stepSeqRow) {
                    const s = stepSeqRow.querySelector(`[data-step-index="${step}"]`)
                    if (s) s.classList.add('active')
      ...
      if ((e.ctrlKey || e.metaKey) && e.code === 'KeyD') {
        ...
        if (selectedClips.size > 0) {
          ...
          clipsToDup.forEach((clip) => {
            ...
            const trackId = isPattern
              ? clone.dataset.patternTrackId
              : clip.parentElement.dataset.trackId || 'drums'

            const oldSeq = isPattern
              ? sequencer.patternClips.find((pc) => pc.uiElement === clip)
              : sequencer.clips.find((c) => c.uiElement === clip)
            if (oldSeq) {
    ...
    setTimeout(async () => {
      try {
        ...
        console.warn('[Browser] Could not auto-load audio folders:', e)
      }
      ...
      try {
        ...
        if (libraryBase) {
          ...
          if (entries && entries.length > 0) {
            ...
            for (const folder of folders) {
              buildBrowserTree(
                installedContainer || browserContainer,
                folder.name,
                folder.path,
                true
              )
        ...
        console.warn('[Browser] Could not auto-load library folders:', e)
      }
    }, 500)
    ...
    if (arrangementView) {
      // Zoom
      arrangementView.addEventListener(
        'wheel',
        (e) => {
      ...
      arrangementView.addEventListener('drop', async (e) => {
        ...
        try {
          ...
          if (buffer) {
            ...
            const trackLane =
              e.target.closest('.track-lane') || document.querySelector('.track-lane')
            if (trackLane) {
        ...
        if (swapIndex !== -1) {
          ...
          if (draggedIndex < swapIndex) {
            ...
            if (laneContainer && draggedLane && swapLane) {
              const insertBeforeLane = swapAutoLane
                ? swapAutoLane.nextSibling
                : swapLane.nextSibling
          ...
          if (mixerContainer && draggedTrackId && swapTrackId) {
            const draggedChannel = mixerContainer.querySelector(
              `.mixer-channel[data-track-id="${draggedTrackId}"]`
            )
            const swapChannel = mixerContainer.querySelector(
              `.mixer-channel[data-track-id="${swapTrackId}"]`
            )
      ...
      document.addEventListener('mouseup', (e) => {
        ...
        if (clipDragState) {
          if (clipDragState.type === 'move') {
            ...
            const sc =
              sequencer.clips.find((c) => c.uiElement === clipDragState.clip) ||
      ...
      arrangementView.addEventListener('contextmenu', (e) => {
        ...
        if (clip) {
          ...
          menu.querySelector('#clip-ctx-rename').addEventListener('click', () => {
            const currentName = isPattern
              ? seqClip.name || 'Pattern'
              : seqClip.name || seqClip._fileName || 'Audio Clip'
    ...
    function selectTrackById(trackId) {
      ...
      document
        .querySelectorAll('.track-header, .mixer-channel, .seq-row')
    ...
    function createTrackUI(trackId, trackName, defaultColor, refTrackId = null, position = null) {
      ...
      trackUIMap[trackId].headers.push({
        mBtn: mBtnH,
        sBtn: sBtnH,
        rBtn: rBtnH,
        nameEl,
      ...
      trackUIMap[trackId].mixers.push({
        mBtn: mBtnM,
        sBtn: sBtnM,
        nameEl: nameElM,
        channelEl: channel,
      ...
      function drawAutoCanvas() {
        ...
        const W = autoCanvas.width,
          H = 60,
          pps = sequencer.pxPerSecond
        ...
        ctx2d.fillStyle = _isLight ? 'rgba(0,0,0,0.03)' : 'rgba(0,0,0,0.3)'
        ctx2d.fillRect(0, 0, W, H)
        // bar grid
        const secPerBar = sequencer.timeSignature.numerator / (sequencer.bpm / 60)
        const pxPerBar = secPerBar * pps
        ctx2d.strokeStyle = _isLight ? 'rgba(0,0,0,0.08)' : 'rgba(255,255,255,0.06)'
        ctx2d.lineWidth = 1
        for (let x = 0; x < W; x += pxPerBar) {
          ctx2d.beginPath()
          ctx2d.moveTo(x, 0)
          ctx2d.lineTo(x, H)
          ctx2d.stroke()
        ...
        if (ctr > 0 && ctr < 1) {
          ctx2d.strokeStyle = _isLight ? 'rgba(0,0,0,0.15)' : 'rgba(255,255,255,0.1)'
          ctx2d.setLineDash([3, 3])
          ctx2d.beginPath()
          ctx2d.moveTo(0, H - ctr * H)
          ctx2d.lineTo(W, H - ctr * H)
          ctx2d.stroke()
          ctx2d.setLineDash([])
        }
        ...
        ctx2d.lineTo(0, H)
        ctx2d.closePath()
        ctx2d.fillStyle = 'rgba(0,229,255,0.07)'
        ctx2d.fill()
        // breakpoints
        sorted.forEach((p) => {
          const x = p.time * pps,
      ...
      autoCanvas.addEventListener('mousedown', (e) => {
        ...
        if (!dragPt) {
          const newPt = {
            time: Math.max(0, mx / pps),
            value: Math.max(min, Math.min(max, min + (1 - my / 60) * (max - min)))
          }
          pts.push(newPt)
      ...
      document.addEventListener('mousemove', (e) => {
        ...
        dragPt.value = Math.max(
          min,
          Math.min(max, min + (1 - (e.clientY - rect.top) / 60) * (max - min))
        )
        getAutoPoints().sort((a, b) => a.time - b.time)
        drawAutoCanvas()
      })
      document.addEventListener('mouseup', () => {
        dragPt = null
      ...
      paramSelect.addEventListener('change', () => {
        currentAutoParam = paramSelect.value
        drawAutoCanvas()
      })
      ...
      header.addEventListener('drop', async (e) => {
        ...
        try {
          ...
          console.error('Instrument drop error:', err)
        }
      })
      ...
      mBtnH.addEventListener('click', toggleMute)
      mBtnM.addEventListener('click', toggleMute)
      sBtnH.addEventListener('click', toggleSolo)
      sBtnM.addEventListener('click', toggleSolo)
    ...
    function shiftTrackDirection(trackId, direction) {
      ...
      if (mixerContainer) {
        const draggedChannel = mixerContainer.querySelector(
          `.mixer-channel[data-track-id="${trackId}"]`
        )
        const swapChannel = mixerContainer.querySelector(
          `.mixer-channel[data-track-id="${swapTrackId}"]`
        )
      ...
      try {
        ...
        const defaultSamples = [
          { tid: 'kick', name: 'kick.wav', path: `${defaultPath}/Starter Pack/Drum Kit/kick.wav` },
          {
            tid: 'snare',
            name: 'snare.wav',
            path: `${defaultPath}/Starter Pack/Drum Kit/snare.wav`
          },
          {
            tid: 'hihat',
            name: 'hihat.wav',
            path: `${defaultPath}/Starter Pack/Drum Kit/hihat.wav`
          },
          {
            tid: 'piano',
            name: 'epiano_C4.wav',
            path: `${defaultPath}/Starter Pack/Keys/epiano_C4.wav`
          },
          {
            tid: 'bass',
            name: 'sub_bass_C2.wav',
            path: `${defaultPath}/Starter Pack/Bass/sub_bass_C2.wav`
          },
          { tid: 'clap', name: 'clap.wav', path: `${defaultPath}/Starter Pack/Drum Kit/clap.wav` },
          {
            tid: 'shaker',
            name: 'shaker.wav',
            path: `${defaultPath}/Starter Pack/Percussion/shaker.wav`
          },
          {
            tid: 'tom_hi',
            name: 'tom_hi.wav',
            path: `${defaultPath}/Starter Pack/Drum Kit/tom_hi.wav`
          },
          {
            tid: 'organ',
            name: 'organ_C4.wav',
            path: `${defaultPath}/Starter Pack/Organ/organ_C4.wav`
          }
        ]
        ...
        console.warn('Could not auto-load instruments', e)
      }
    }, 800)
    ...
    function renderLoop() {
      ...
      if (posDisplay) {
        const timeInSeconds = sequencer.isPlaying
          ? engine.ctx.currentTime - sequencer.startTime
          : sequencer.pauseTime
      ...
      allMixerChannels.forEach((channel) => {
        ...
        let peak = 0
        if (
          channel.classList.contains('master') ||
          channel.querySelector('.channel-name-vertical, .channel-name')?.textContent === 'Master'
        ) {
    ...
    document.getElementById('position-display')?.addEventListener('click', () => {
      ...
      showToast(
        lcdDisplayMode === 'bars' ? '⏱ Position: Bars · Beats · Ticks' : '⏱ Position: MM : SS . ms'
      )
    })
    ...
    function buildStepRows(count, targetContainer = null) {
      const containers = targetContainer
        ? [targetContainer]
        : document.querySelectorAll('.seq-steps')
        ...
        if (row) {
          ...
          if (active && !existing) {
            ...
            n.addEventListener('click', (e) => {
              ...
              const stepSeqRow = document.querySelector(
                `.seq-steps[data-instrument="${trackId}"][data-note="${noteName}"]`
              )
              if (stepSeqRow) {
        ...
        if (prDragState) {
          ...
          if (isResize) {
            const finalDur = parseFloat(
              prDragState.noteElement.dataset.duration || prDragState.startDur
            )
            sequencer.removeNoteFromPattern(
              prDragState.trackId,
              prDragState.noteName,
              prDragState.startStep
            )
            sequencer.addNoteToPattern(
              prDragState.trackId,
              prDragState.noteName,
              prDragState.startStep,
              finalDur
            ...
            const stepSeqRow = document.querySelector(
              `.seq-steps[data-instrument="${prDragState.trackId}"][data-note="${prDragState.noteName}"]`
            )
            if (stepSeqRow) {
            ...
            prDragState.items?.forEach((item) => {
              ...
              if (!prDragState.isNew) {
                ...
                const oldStepSeqRow = document.querySelector(
                  `.seq-steps[data-instrument="${prDragState.trackId}"][data-note="${item.noteName}"]`
                )
                if (oldStepSeqRow) {
              ...
              const stepSeqRow = document.querySelector(
                `.seq-steps[data-instrument="${prDragState.trackId}"][data-note="${finalNoteName}"]`
              )
              if (stepSeqRow) {
      ...
      if (selectedNotes.size > 0) {
        ...
        return notes
          .map((n) => {
            ...
            const el = row
              ? Array.from(row.querySelectorAll('.pr-note')).find(
                  (e) => parseInt(e.dataset.step) === n.step
                )
              : null
            return {
              el,
    ...
    document.getElementById('pr-quantize-btn')?.addEventListener('click', () => {
      ...
      notesToProcess.forEach((n) => {
        ...
        if (newStep !== n.step) {
          ...
          const oldStepSeqRow = document.querySelector(
            `.seq-steps[data-instrument="${currentTrackId}"][data-note="${n.noteName}"]`
          )
          if (oldStepSeqRow) {
          ...
          const newStepSeqRow = document.querySelector(
            `.seq-steps[data-instrument="${currentTrackId}"][data-note="${n.noteName}"]`
          )
          if (newStepSeqRow) {
    ...
    document.getElementById('pr-chop-btn')?.addEventListener('click', () => {
      ...
      const map = trackUIMap[currentTrackId]
      const color =
        map && map.headers[0] ? map.headers[0].laneEl.style.getPropertyValue('--track-color') : null
      const grid = document.getElementById('piano-grid')
    ...
    if (pianoRollView) {
      pianoRollView.addEventListener(
        'wheel',
        (e) => {
          if (e.ctrlKey || e.metaKey) {
            e.preventDefault()
    ...
    function serializeProject() {
      ...
      const headers = Array.from(
        document.querySelectorAll('#track-headers-container .track-header')
      )
      headers.forEach((h) => {
        const trackId = Object.keys(trackUIMap).find(
          (tid) => trackUIMap[tid].headers[0]?.headerEl === h
        )
        if (!trackId) return
        const map = trackUIMap[trackId]
        const engineTrack = engine.getTrack(trackId)
    ...
    function doCopy() {
      ...
      if (selectedNotes.size > 0) {
        ...
        selectedClips.forEach((clipEl) => {
          const isPattern = clipEl.classList.contains('pattern-clip')
          const trackId = isPattern
            ? clipEl.dataset.patternTrackId
            : clipEl.parentElement.dataset.trackId || 'drums'
          const start = parseFloat(clipEl.dataset.startTime) || 0
    ...
    function doPaste() {
      ...
      const currentPlayheadTime = sequencer.isPlaying
        ? engine.ctx.currentTime - sequencer.startTime
        : sequencer.pauseTime
      ...
      if (dawClipboard.type === 'notes') {
        ...
        dawClipboard.data.forEach((item) => {
          ...
          if (row) {
            ...
            const stepSeqRow = document.querySelector(
              `.seq-steps[data-instrument="${currentTrackId}"][data-note="${item.noteName}"]`
            )
            if (stepSeqRow) {
              const s = stepSeqRow.querySelector(`[data-step-index="${finalStep}"]`)
              if (s) s.classList.add('active')
        ...
        dawClipboard.data.forEach((item) => {
          ...
          if (lane) {
            if (item.isPattern) {
              const clipEl = createPatternClipUI(
                item.trackId,
                finalStartTime,
                item.duration,
                lane,
    ...
    document.getElementById('menu-new')?.addEventListener('click', () => {
      ...
      document
        .getElementById('step-sequencer-view')
        ...
        for (const track of data.tracks) {
          ...
          if (track.instrumentPath) {
            try {
              ...
              console.warn('Failed to load instrument during restore:', track.id, e)
            }
          }
      ...
      if (data.clips) {
        for (const c of data.clips) {
          ...
          try {
            ...
            console.warn('Could not restore clip:', err)
          }
        }
      }
    ...
    async function patchProject(data) {
      ...
      for (const track of targetTracks) {
        ...
        if (engineTrack) {
          ...
          if (track.instrumentPath) {
            if (engineTrack._instrumentPath !== track.instrumentPath) {
              try {
                ...
                console.warn('Failed to load cached instrument during restore:', track.id, e)
              }
            }
          }
          ...
          const fxDiffers =
            targetEffects.length !== currentEffects.length ||
            targetEffects.some(
              (fx, i) =>
                !currentEffects[i] ||
                currentEffects[i].id !== fx.id ||
                currentEffects[i].type !== fx.type
            )
      ...
      const masterFxDiffers =
        targetMasterFX.length !== currentMasterFX.length ||
        targetMasterFX.some(
          (fx, i) =>
            !currentMasterFX[i] ||
            currentMasterFX[i].id !== fx.id ||
            currentMasterFX[i].type !== fx.type
        )
      ...
      const newClips = targetClips.filter(
        (tc) =>
          !clipsToKeep.some(
            (cc) =>
      ...
      for (const c of newClips) {
        ...
        try {
          let absolutePath = c.filePath
          const isRelative =
            !c.filePath.startsWith('/') && !c.filePath.startsWith('\\') && !c.filePath.includes(':')
          if (isRelative) {
            let baseDir = ''
            if (currentProjectPath) {
              const lastIndex = Math.max(
          ...
          console.warn('Failed to patch clip:', e)
        }
      }
      ...
      sequencer.patternClips.forEach((c) => {
        const match = targetPatternClips.find(
          (tc) =>
      ...
      const newPatternClips = targetPatternClips.filter(
        (tc) =>
          !patternClipsToKeep.some(
            (cc) =>
              cc.trackId === tc.trackId &&
              Math.abs(cc.startTime - tc.startTime) < 0.001 &&
              Math.abs(cc.duration - tc.duration) < 0.001
    ...
    function showPluginWindow(fxObj) {
      ...
      if (fxObj.params) {
        Object.keys(fxObj.params).forEach((paramName) => {
          ...
          if (paramName === 'depth' && (fxObj.type === 'flanger' || fxObj.type === 'chorus')) {
            min = 0.0001
            max = 0.015
          }
          if (paramName === 'delay') {
          ...
          // Phaser
          if (paramName === 'depth' && fxObj.type === 'phaser') {
            min = 0
            max = 4000
          }
          if (paramName === 'baseFreq' && fxObj.type === 'phaser') {
            min = 100
            max = 8000
          }
          // Limiter
          ...
          if (paramName === 'normfreq') {
            min = 0.001
            max = 1.0
          }
          // Auto-Wah
          if (paramName === 'baseFreq' && fxObj.type === 'autowah') {
            min = 20
            max = 2000
          }
          if (paramName === 'sensitivity') {
            min = 0.1
            max = 10
          }
          if (paramName === 'Q') {
          ...
          // Delay time
          if (paramName === 'time' && fxObj.type === 'delay') {
            min = 0.001
            max = 5.0
          }
          // Distortion amount
          if (paramName === 'amount' && fxObj.type === 'distortion') {
            min = 0
            max = 1.0
          }
        ...
        content.innerHTML =
          '<div style="color: var(--text-secondary); font-size: 0.8rem;">No parameters available.</div>'
      }
    ...
    function showGeneratorWindow(trackId) {
      ...
      paramsSpec.forEach((pSpec) => {
        ...
        if (pSpec.isOscSelect) {
          ...
          currentVal =
            track.generator.params[paramName] !== undefined
              ? track.generator.params[paramName]
              : min
        }
              ...
              if (currentTrackId !== 'master') {
                ...
                if (currentTrack) {
                  ...
                  currentTrack.generator = {
                    type: item.id,
                    params:
                      currentTrack.generator?.type === item.id
                        ? currentTrack.generator.params || defaultParams
                        : defaultParams
                  }
                  showGeneratorWindow(currentTrackId)
                  window.updatePluginsFooter()
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (e.target.tagName !== 'INPUT' && e.target.tagName !== 'TEXTAREA') {
        if (
          ...
          !e.metaKey &&
          e.key.toLowerCase() === 'q' &&
          document.getElementById('piano-roll-view')?.classList.contains('active')
        ) {
          e.preventDefault()
    ...
    async function openPreferencesModal() {
      ...
      if (latencySelect)
        latencySelect.value = localStorage.getItem('bounce.latencyHint') || 'interactive'
      ...
      if (autosaveInterval)
        autosaveInterval.value = localStorage.getItem('bounce.autoSaveInterval') || '5'
      ...
      if (followCheck)
        followCheck.checked = localStorage.getItem('bounce.followPlayhead') !== 'false'
      ...
      if (monitorCheck)
        monitorCheck.checked = localStorage.getItem('bounce.inputMonitor') === 'true'

      const autoNameCheck = document.getElementById('pref-auto-name-clips')
      if (autoNameCheck)
        autoNameCheck.checked = localStorage.getItem('bounce.autoNameClips') !== 'false'
    ...
    async function encodeToMp3(audioBuffer, numChannels, bitrate, progressCallback) {
      ...
      const mp3encoder = new lamejs.Mp3Encoder(isStereo ? 2 : 1, sampleRate, bitrate || 192)
      const mp3Chunks = []
    ...
    exportStartBtn?.addEventListener('click', async () => {
      ...
      const formatVal = document.getElementById('export-format').value // 'wav-16', 'wav-24', 'wav-32', 'webm', 'mp3-192', 'mp3-128', 'mp3-64'
      ...
      try {
        if (!currentProjectPath) {
          showToast('Please save your project first to create a project folder.')
          resetExportUI()
          document.getElementById('menu-save')?.click()
          return
        }

        const { projectDir } = resolveProjectFolderAndFile(currentProjectPath)

        const renderedBuffer = await engine.renderProjectOffline(
          sequencer,
          {
            sampleRate,
        ...
        if (formatVal.startsWith('wav-')) {
          ...
          const filePath = `${projectDir}/Mixdown.wav`
          ...
          showToast(`Exported WAV (${bitDepth}-bit) successfully to project folder!`)
          closeExportModal()
        } else if (formatVal === 'webm') {
          progressText.textContent = 'Encoding Opus (Silent Capture)...'

          const webmBlob = await captureWebmSilently(
            renderedBuffer,
            sampleRate,
            channels,
            (percent) => {
          ...
          const filePath = `${projectDir}/Mixdown.webm`
          ...
          showToast(`Exported WebM Opus successfully to project folder!`)
          closeExportModal()
        } else if (formatVal.startsWith('mp3-') || formatVal === 'mp3') {
          const bitrate = formatVal.startsWith('mp3-') ? parseInt(formatVal.split('-')[1]) : 192
          progressText.textContent = `Encoding MP3 (${bitrate} kbps)...`
          const mp3Blob = await encodeToMp3(renderedBuffer, channels, bitrate, (percent) => {
            progressPercent.textContent = `${percent}%`
            progressBar.style.width = `${percent}%`
          })

          progressText.textContent = 'Writing MP3 file...'
          const filePath = `${projectDir}/Mixdown.mp3`

          const arrayBuf = await mp3Blob.arrayBuffer()
          const uint8Arr = new Uint8Array(arrayBuf)
          await window.api.writeFile(filePath, uint8Arr)
          showToast(`Exported MP3 (${bitrate} kbps) successfully to project folder!`)
          closeExportModal()
        }
      } catch (err) {
    ...
    prefApplyBtn?.addEventListener('click', async () => {
      ...
      if (autoNameCheck)
        localStorage.setItem('bounce.autoNameClips', autoNameCheck.checked.toString())
...
function buildPianoRoll() {
  ...
  grid.addEventListener('scroll', () => {
    keysContainer.scrollTop = grid.scrollTop
  })
  keysContainer.addEventListener('scroll', () => {
    grid.scrollTop = keysContainer.scrollTop
  })
}
