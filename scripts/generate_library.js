    ...
    for (let i = 0; i < numSamples; i++) {
        ...
        if (type === 'rimshot') {
            ...
            for (let m = 0; m < modes.length; m++) {
                val += weights[m] * Math.sin(2 * Math.PI * freq * modes[m] * t) * Math.exp(-t * (15 + m * 10));
            }
            // Strike transient
...
function generate808(type) {
    ...
    for (let i = 0; i < numSamples; i++) {
        ...
        if (type === 'kick') {
            ...
            const f1 = 540, f2 = 800;
            val = (Math.sign(Math.sin(2 * Math.PI * f1 * t)) + Math.sign(Math.sin(2 * Math.PI * f2 * t))) * 0.5;
            val *= Math.exp(-t * 15);
        }
...
// Brass
writeWav(`${baseDir}/Brass/trumpet_hit.wav`, generateBrass(440, {
    duration: 0.4,
    attack: 0.01,
    decay: 0.1,
    sustain: 0.1,
...
writeWav(`${baseDir}/Brass/trombone_low.wav`, generateBrass(110, {
    duration: 1.2,
    attack: 0.05,
    decay: 0.3,
    sustain: 0.4,
...
writeWav(`${baseDir}/Brass/horn_stabs.wav`, generateBrass(220, {
    duration: 0.6,
    attack: 0.03,
    decay: 0.15,
    sustain: 0.2,
...
writeWav(`${baseDir}/Brass/tuba_low.wav`, generateBrass(55, {
    duration: 1.5,
    attack: 0.08,
    decay: 0.4,
    sustain: 0.5,
...
writeWav(`${baseDir}/Brass/brass_ensemble.wav`, generateBrass(220, {
    duration: 2.0,
    attack: 0.1,
    decay: 0.5,
    sustain: 0.6,
...
writeWav(`${baseDir}/Drum Kit/snare_electronic.wav`, generateDrum('snare', 'electronic'), sampleRate);
...
// Hits
writeWav(`${baseDir}/Hits/bass_hit_deep.wav`, generateBassHit({
    freq: 45,
    decay: 3,
    pitchDrop: 0.3,
    noiseAmount: 0.05
...
writeWav(`${baseDir}/Hits/bass_hit_808.wav`, generateBassHit({
    freq: 55,
    decay: 1.5,
    pitchDrop: 1.0,
    noiseAmount: 0.1,
...
writeWav(`${baseDir}/Hits/bass_hit_heavy.wav`, generateBassHit({
    freq: 40,
    decay: 5,
    pitchDrop: 0.2,
    distortion: 0.8,
...
writeWav(`${baseDir}/Hits/bass_hit_cinematic.wav`, generateBassHit({
    freq: 35,
    decay: 6,
    pitchDrop: 0.1,
    noiseAmount: 0.3,
